...
/**
 * Named rather than asserted with `!` because the row going missing mid-poll is
 * a different failure from the worker leaving it in the wrong state: `jobs`
 * cascades from `analyses`, so a truncate elsewhere takes it with it, and
 * "cannot read status of null" says nothing about which of the two happened.
 */
async function statusOf(jobId: string): Promise<string> {
  const job = await findJob(jobId);
  if (job === null) throw new Error(`Job ${jobId} disappeared while the test was watching it.`);
  return job.status;
}
