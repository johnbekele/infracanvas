/**
 * Validation for user-supplied values interpolated into GitHub API URLs.
 *
 * These handlers proxy requests using the caller's OAuth token as a bearer
 ...
 * token.
 *
 * This mirrors `apps/api/src/lib/github-params.ts`. The duplication is
 * deliberate and temporary: this entire directory is a copy of the Express API
 * kept alive for the Vercel deployment, and it is removed in Epic 1. Both copies
 * must be fixed while both are reachable.
 */
