/**
 * SHA-256 over a string, in TypeScript.
 *
 * `packages/core` is bundled into the browser as well as run under Node, so
 * `node:crypto` is not available to it, and `crypto.subtle.digest` is
 * asynchronous in both environments while `irDigest` is specified as a pure
 * synchronous function. A digest here identifies content rather than
 * authenticating it, so the requirement is determinism and portability rather
 * than a constant-time primitive.
 *
 * Correctness is pinned to the published FIPS 180-4 vectors in `sha256.test.ts`
 * rather than to review of the arithmetic below.
 */
...
/** The message, a `0x80` terminator, zero padding, and the bit length as a big-endian 64-bit integer. */
function pad(message: Uint8Array): DataView {
  ...
  // Split rather than using BigInt: a document long enough to overflow 2^53
  // bits is 10^15 characters, which no caller of this package can produce.
...
export function sha256Hex(text: string): string {
  const view = pad(new TextEncoder().encode(text));
